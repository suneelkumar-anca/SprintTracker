...
export default function SavedReportCard({ report, idx, total, onLoad, onDelete, onExport, onPublishReport, onPublishRetro, onMoveUp, onMoveDown, onDragStart, onDragOver, onDrop, onDragEnd, isDragging, isOver }) {
  ...
  return (
    <div draggable aria-label={`${report.name}, position ${idx + 1} of ${total}`} aria-describedby={`drag-hint-${report.id}`}
      ...
      <SavedReportActions report={report} onLoad={onLoad} onExport={onExport} onDelete={onDelete} onPublishReport={onPublishReport} onPublishRetro={onPublishRetro} onMoveUp={onMoveUp} onMoveDown={onMoveDown} />
    </div>
  );
}
